            ...
            loop {
                ...
                if is_avc {
                    ...
                    if let (Some(encoder), Some(sender)) = (&mut video_encoder, &egfx_sender) {
                        ...
                        // unscaled at its own geometry. The locals keep the
                        // post-ladder code independent of which space the
                        // damage regions were produced in.
                        ...
                        let mut damage_regions = if force_full_frame {
                            ...
                            // NOTE: detection and hints run on the raw capture buffer
                            // (== desktop space for kwin-virtual), so regions arrive in
                            // desktop space and need no further mapping.
                        ...
                        if should_log_telemetry {
                            ...
                            // Cross-check from the calibration probe: the
                            // probe already measured the pixel-diff ratio for
                            // this same frame (see compositor-hint arm above),
                            // so reuse it rather than re-diffing the hints.
                            if let Some(pixel_diff_ratio) = compositor_vs_pixel_diff {
                                ...
                                let delta_pct = (hint_ratio - pixel_diff_ratio) * 100.0;
                                debug!(
